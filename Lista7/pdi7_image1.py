# -*- coding: utf-8 -*-
"""
Created on Mon Oct  7 17:36:36 2019
...
def findNextB_C_4chain(img, b, c):
    caminho = {(b[0]-1,b[1]-1):(b[0]-1,b[1]),
               (b[0]-1,b[1]):(b[0]-1,b[1]+1),
               (b[0]-1,b[1]+1):(b[0],b[1]+1),
               (b[0],b[1]+1):(b[0]+1,b[1]+1),
    ...
    for temp in range(8):
        coord = caminho[coord_atual]
        if img[coord] == 1:
            if not isDiagonal(b, coord):
#                if img[coord_atual] == 1:
#                    coord_atual = coord_atual_diag
                return coord, coord_atual
            else:
                if coord_diag == 0:
                    coord_diag = coord
                    coord_atual_diag = coord_atual
        coord_atual = coord
    
    return coord_diag, coord_atual_diag
...
if __name__ == "__main__":
    ...
    if run_all:
        border_img, coord_list = fronteira("images/image_1.png", "border_image1.png")
        resample_img, resample_list, grid = resample(border_img, coord_list, min_dist, grid_value)
    ...
    chain_code4_normPosicao, points_c_code_4_normPosicao = normalize_position(chain_code4, points_c_code_4)
    chain_code4_normPosicao_normRotation = normalize_rotation(chain_code4_normPosicao, chain_type=4)
    
    
    chain_code8, points_c_code_8, chainCodeMatrix_8 = chain_code(resample_list, grid_value, chain_type=8)
    chain_code8_normPosicao, points_c_code_8_normPosicao = normalize_position(chain_code8, points_c_code_8)
    chain_code8_normPosicao_normRotation = normalize_rotation(chain_code8_normPosicao, chain_type=8)
    
    
#    for c in code_points:
#        chainCodeMatrix[c] = 3
    
#    b0_norm, code_norm = normalize_position(code, code_points)
    
#    chain_border_img, chain_coord_list = fronteira("chainCodeMatrix.png", "fronteira_chainCode.png")
